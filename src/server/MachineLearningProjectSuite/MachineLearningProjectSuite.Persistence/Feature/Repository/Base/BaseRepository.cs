...
namespace MachineLearningProjectSuite.Persistence.Feature.Repository.Base
{
        ...
        where TKey : IComparable
    {
        ...
        public virtual async Task<IList<TEntity>> GetAsync(object selector,
            Expression<Func<TEntity, bool>>? filter = null,
            Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>>? orderBy = null,
            Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>>? include = null,
            bool isTrackingOff = false)
        ...
        public async Task<TEntity?> SingleAsync(Expression<Func<TEntity, bool>> filter, bool isTrack = true)
        {
            var result = isTrack ? await _dbSet.FirstOrDefaultAsync(filter) : await _dbSet.AsNoTracking().FirstOrDefaultAsync(filter);
            return result;
        }

        public async Task<TEntity?> SingleAsync(Expression<Func<TEntity, bool>> filter, Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>> include, bool isTrack = true)
        {
            IQueryable<TEntity> query = _dbSet;
            query = include(query);
            var result = isTrack ? await query.FirstOrDefaultAsync(filter) : await query.AsNoTracking().FirstOrDefaultAsync(filter);
            return result;
        }
        ...
        public virtual async Task EditAsync(TEntity entityToUpdate, CancellationToken cancellationToken)
        {
            await Task.Run(() =>
            {
                _dbSet.Attach(entityToUpdate);
